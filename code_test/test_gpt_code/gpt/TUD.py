...
COLORS = [plt.rcParams["axes.prop_cycle"].by_key()["color"][i] for i in range(8)] + 5 * mcp.gen_color(cmap="tab20b",
                                                                                                      n=20)
...
class UAM(gym.Env):
    ...
    def _spawn_plane(self, n: int = None, random: bool = False):
        ...
        p = Plane(role="RL", dt=self.dt, actype=self.actype, lat=lat, lon=lon, alt=self.acalt, hdg=(hdg + 180) % 360,
                  tas=tas)
    ...
    def _get_state(self, i: int) -> np.ndarray:
        ...
        abs_bng_goal, d_goal = qdrdist(latd1=p.lat, lond1=p.lon, latd2=self.dest.lat,
                                       lond2=self.dest.lon)  # outputs ABSOLUTE bearing
        ...
        if self.N_planes > 1:
            ...
            if (not self.multi_policy):
                ...
                TS_info = np.pad(TS_info, (0, desired_length - len(TS_info)), 'constant',
                                 constant_values=np.nan).astype(np.float32)
    ...
    def render(self, mode=None):
        ...
        if self.step_cnt % 1 == 0:
            ...
            if self.plot_reward:
                if self.step_cnt == 0:
                    ...
                    if self.multi_policy:
                        ...
                        self.ax2.r_coll[self.step_cnt] = self.r_coll if isinstance(self.r_coll, float) else float(
                            self.r_coll[0])
                        self.ax2.r_goal[self.step_cnt] = self.r_goal if isinstance(self.r_goal, float) else float(
                            self.r_goal[0])
            ...
            if self.step_cnt % 50 == 0:
                ...
                # spawning area
                lats, lons = map(list, zip(*[
                    qdrpos(latd1=self.dest.lat, lond1=self.dest.lon, qdr=deg, dist=meter_to_NM(self.dest.spawn_radius)) \
                    for deg in self.clock_degs]))
                self.ax1.plot(lons, lats, color="grey")

                # respawn area
                lats, lons = map(list, zip(*[qdrpos(latd1=self.dest.lat, lond1=self.dest.lon, qdr=deg,
                                                    dist=meter_to_NM(self.dest.respawn_radius)) \
                                             for deg in self.clock_degs]))
                ...
                # destination
                lats, lons = map(list, zip(*[
                    qdrpos(latd1=self.dest.lat, lond1=self.dest.lon, qdr=deg, dist=meter_to_NM(self.dest.radius)) \
                    for deg in self.clock_degs]))
                ...
                for i, p in enumerate(self.planes):
                    ...
                    # information
                    self.ax1.txts.append(
                        self.ax1.text(x=0.0, y=0.0, s="", color=COLORS[i], fontdict={"size": 8}, animated=True))
                ...
                if self.plot_reward:
                    ...
                    if self.multi_policy:
                        for i in range(self.N_planes):
                            self.ax2.lns_agg.append(
                                self.ax2.plot([], [], color=COLORS[i], label=f"Agg {i}", animated=True)[0])
                            self.ax2.lns_coll.append(
                                self.ax2.plot([], [], color=COLORS[i], label=f"Collision {i}", linestyle="dotted",
                                              animated=True)[0])
                            self.ax2.lns_goal.append(
                                self.ax2.plot([], [], color=COLORS[i], label=f"Goal {i}", linestyle="dashed",
                                              animated=True)[0])
                    else:
                        self.ax2.lns_agg.append(self.ax2.plot([], [], color=COLORS[0], label=f"Agg", animated=True)[0])
                        self.ax2.lns_coll.append(
                            self.ax2.plot([], [], color=COLORS[1], label=f"Collision", animated=True)[0])
                        self.ax2.lns_goal.append(
                            self.ax2.plot([], [], color=COLORS[2], label=f"Goal", animated=True)[0])
                ...
                if self.plot_state:
                    ...
                    for i in range(self.obs_size):
                        self.ax3.lns.append(
                            self.ax3.plot([], [], label=self.obs_names[i], color=COLORS[i], animated=True)[0])
                    self.ax3.legend()
                ...
                for i, p in enumerate(self.planes):
                    ...
                    # incident area
                    lats, lons = map(list, zip(*[
                        qdrpos(latd1=p.lat, lond1=p.lon, qdr=deg, dist=meter_to_NM(self.incident_dist / 2)) \
                        for deg in self.clock_degs]))
                ...
                if self.plot_reward:
                    if self.multi_policy:
                        for i in range(self.N_planes):
                            self.ax2.lns_agg[i].set_data(np.arange(self.step_cnt + 1),
                                                         self.ax2.r[i][:self.step_cnt + 1])
                            self.ax2.lns_coll[i].set_data(np.arange(self.step_cnt + 1),
                                                          self.ax2.r_coll[i][:self.step_cnt + 1])
                            self.ax2.lns_goal[i].set_data(np.arange(self.step_cnt + 1),
                                                          self.ax2.r_goal[i][:self.step_cnt + 1])
